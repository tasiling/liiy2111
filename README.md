...
- 因本次執行環境的網路出口政策封鎖 `api.notion.com`(僅 Notion 官方 MCP 連接器可連,App 本身的伺服器端請求連不出去),**本期未能在此環境內對 Notion 執行過實際寫入操作**,僅完成程式碼層級驗證與 schema 讀取驗證(透過 MCP 連接器完成)。擁有者部署到 Railway/Vercel 等有正常網路的環境後,必須先用測試資料實際跑過一次「批次建立 14 天 Session」「序列展開預覽+確認寫入」再正式使用,驗證後記得刪除測試筆。
